...
network_save=1; network_print=1
epidemic_save=1; epidemic_print=1
...
for row in range(rowstart,rowend+1):
     ...
     Plinkrow=Plink[row];  betarow=bbeta[row]; betahrow=bbeta[row]*h[row]
     for i in range(NTiter):  # going through the chunks i=0,1,...,NTiter-1
          ...
          nzc=cp.count_nonzero(Children_mtx_chunk).get().item() # count how many nonzeros
     ...
     if network_print:
          ...
          sC= cp.ravel(sparse.spmatrix.sum(CSP, axis=0)+cp.transpose(sparse.spmatrix.sum(CSP, axis=1)))
          C_P_hist = plt.figure(1)
          plt.hist([sC.get()], bins=range(int(cp.amax(sC))+2), align='left',rwidth=0.9, label=["Children"])
     ...
     for i in range(N):
          neighbs1=cp.concatenate((CSP.getcol(i).tocsc().indices, CSP.getrow(i).indices))
     ...
     for i_house in cp.nonzero(Children0)[0]:
          if nneighbs[i_house]:
               for r0loop in range(r0_repeat):
                    # initialization
                    Children=cp.copy(Children0);  Susceptible=cp.copy(Children0)
                    Pregnancy=cp.copy(Pregnancy0);   Recovered.fill(0); 
                    AllInfected.fill(0);  AllInfected[i_house]=1
                    Infected.fill(0); Infected[i_house,0]=1; 
                    Infected_Total=0; InfNeighb.fill(0); P_Infection=0.0
     ...
     R0[row]=R0[row]/(r0_repeat*N)
     print("row:",row,"Plink:",Plink[row],"beta:",betarow,"R0:", R0[row])


endtime = time.time()
print("seconds/network: ", round((endtime - starttime))) #/(rowend+1-rowstart)))
